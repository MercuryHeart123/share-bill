import { IonButton, IonCard, IonChip, IonCol, IonContent, IonIcon, IonInput, IonItem, IonLabel, IonModal, IonRow } from "@ionic/react";
import { add, checkmark, personAdd, remove } from "ionicons/icons";
...
const ItemModal = ({ itemModalOpen, setItemModalOpen, persons, setPersons, setItems, editingIndex, currentEdit, setCurrentEditItem, addPersonName, setAddPersonName }: ItemModalProps) => {
    return (
        <IonModal
        ...
        >
            <IonContent className="ion-padding" scrollEvents={true} fullscreen={true} scrollY={true}>
                <IonCard
                ...
                >
                    <h2 style={{ textAlign: "center", marginBottom: "1rem" }}>แก้ไขรายการ</h2>
                        ...
                        <>
                            <IonItem lines="none" style={{ padding: "0px" }}>
                                ...
                                {`(${currentEdit?.persons.length} คน คนละ ${
                                    currentEdit?.sum === 0 || currentEdit?.persons.length === 0
                                        ? "0"
                                        : ((currentEdit?.sum || 0) / currentEdit?.persons.length).toLocaleString("en-US", {
                                              minimumFractionDigits: 0,
                                              maximumFractionDigits: 2,
                                          })
                                } บาท)`}
                            ...
                            <IonItem lines="full">
                                ...
                                >
                                    ...
                                    >
                                        {persons.map((person) => {
                                            const isPersonInList = currentEdit?.persons.includes(person.id);
                                            return (
                                                <IonChip
                                                    key={person.id}
                                                    style={{
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        setCurrentEditItem((prev) =>
                                                            prev
                                                                ? {
                                                                      ...prev,
                                                                      persons: prev.persons.includes(person.id) ? prev.persons.filter((id) => id !== person.id) : [...prev.persons, person.id],
                                                                  }
                                                                : null
                                                        );
                                                    }}
                                        ...
                                        onClick={() => {
                                            setCurrentEditItem((prev) =>
                                                prev
                                                    ? {
                                                          ...prev,
                                                          persons: prev.persons.length === persons.length ? [] : persons.map((person) => person.id),
                                                      }
                                                    : null
                                            );
                                        }}
                                    >
                                        <IonIcon icon={currentEdit?.persons.length === persons.length ? remove : add} />
                                        {"  "}
                                        {currentEdit?.persons.length === persons.length ? "ยกเลิกเลือกทั้งหมด" : "เลือกทั้งหมด"}
                                    </IonButton>
                                </div>
                            </IonItem>
                        </>
                    ...
                    <IonRow>
                        <IonCol size="9" sizeMd="9" className="ion-text-center ion-align-self-center ion-justify-content-center d-flex">
                            <IonInput
                                value={addPersonName}
                                type="text"
                                inputMode="text"
                        ...
                        <IonCol size="3" sizeMd="3" className="ion-text-center ion-align-self-center ion-justify-content-center d-flex">
                            <IonButton
                                expand="full"
                                className="ion-text-center"
                                onClick={() => {
                ...
                <IonRow style={{ marginTop: "1.5rem" }}>
                    <IonCol size="6" sizeMd="6" className="ion-text-center ion-align-self-center ion-justify-content-center d-flex">
                        {" "}
                        <IonButton
                            expand="block"
                            color={"danger"}
                            onClick={() => {
                                setItems((prev) => prev.filter((_, idx) => idx !== editingIndex));
                    ...
                    <IonCol size="6" sizeMd="6" className="ion-text-center ion-align-self-center ion-justify-content-center d-flex">
                        <IonButton
                            expand="block"
                            onClick={() => {
                                setItems((prev) => prev.map((it, i) => (i === editingIndex ? currentEdit! : it)));
                                setItemModalOpen(false);
                            }}
                        >
                            บันทึก
